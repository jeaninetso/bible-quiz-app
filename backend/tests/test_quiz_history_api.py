...
@pytest.fixture()
def ruth_book_id(db_session, seeded_books):
    return next(b for b in crud.list_books(db_session) if b.code == "Ruth").id


def _create_attempt(monkeypatch, client, book_id) -> int:
...
def test_history_excludes_in_progress_attempts(monkeypatch, logged_in_client, ruth_book_id):
    _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
...
def test_history_lists_completed_attempts_newest_first(monkeypatch, logged_in_client, ruth_book_id):
    first_id = _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
    logged_in_client.post(f"/quiz-attempts/{first_id}/submit", json={"answers": [0, 1, 2, 3, 0]})

    second_id = _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
...
def test_history_only_shows_the_current_users_attempts(monkeypatch, logged_in_client, db_session, ruth_book_id):
    attempt_id = _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
...
def test_review_returns_full_answer_key_for_a_completed_attempt(monkeypatch, logged_in_client, ruth_book_id):
    attempt_id = _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
    logged_in_client.post(f"/quiz-attempts/{attempt_id}/submit", json={"answers": [0, 1, 2, 3, 0]})
...
def test_review_409s_for_an_unsubmitted_attempt(monkeypatch, logged_in_client, ruth_book_id):
    attempt_id = _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
...
def test_review_404s_for_another_users_attempt(monkeypatch, logged_in_client, db_session, ruth_book_id):
    attempt_id = _create_attempt(monkeypatch, logged_in_client, ruth_book_id)
    logged_in_client.post(f"/quiz-attempts/{attempt_id}/submit", json={"answers": [0, 1, 2, 3, 0]})
